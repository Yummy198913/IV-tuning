...
@TRANSFORMS.register_module()
class Resize_vi_ir(MMCV_Resize):
    """Resize images & bbox & seg.
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.scale:
            ...
            img_shape = results['img_vi'].shape[:2]
            results['scale'] = _scale_size(img_shape[::-1], self.scale_factor)
        self._resize_img_vi_ir(results)
        self._resize_bboxes(results)
        self._resize_masks(results)
        self._resize_seg(results)
        self._record_homography_matrix(results)
...
@TRANSFORMS.register_module()
class RandomFlip_vi_ir(MMCV_RandomFlip):
    """Flip the image & bbox & mask & segmentation map. Added or Updated keys:
    flip, flip_direction, img, gt_bboxes, and gt_seg_map. There are 3 flip
    modes:
    ...
    def _flip(self, results: dict) -> None:
        """Flip images, bounding boxes, and semantic segmentation map."""
        # flip image
        results['img_vi'] = mmcv.imflip(
            results['img_vi'], direction=results['flip_direction'])
...
@TRANSFORMS.register_module()
class RandomCrop_vi_ir(BaseTransform):
    """Random crop the image & bboxes & masks.
    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
        ...
        """
        assert crop_size[0] > 0 and crop_size[1] > 0
        img_vi = results['img_vi']
        img_ir = results['img_ir']
        margin_h = max(img_vi.shape[0] - crop_size[0], 0)
        margin_w = max(img_vi.shape[1] - crop_size[1], 0)
        offset_h, offset_w = self._rand_offset((margin_h, margin_w))
        crop_y1, crop_y2 = offset_h, offset_h + crop_size[0]
        crop_x1, crop_x2 = offset_w, offset_w + crop_size[1]
        ...
        # crop the image
        img_vi = img_vi[crop_y1:crop_y2, crop_x1:crop_x2, ...]
        img_ir = img_ir[crop_y1:crop_y2, crop_x1:crop_x2, ...]
        img_shape = img_vi.shape
        results['img_vi'] = img_vi
    ...
    def transform(self, results: dict) -> Union[dict, None]:
        ...
        Returns:
            results (Union[dict, None]): Randomly cropped results, 'img_shape'
                ...
                be returned when there is no valid bbox after cropping.
        """
        image_size = results['img_vi'].shape[:2]
        crop_size = self._get_crop_size(image_size)
        results = self._crop_data(results, crop_size, self.allow_negative_crop)
        return results
...
@TRANSFORMS.register_module()
class CopyPaste_vi_ir(BaseTransform):
    """Simple Copy-Paste is a Strong Data Augmentation Method for Instance
    Segmentation The simple copy-paste transform steps are as follows:
    ...
    def _copy_paste(self, dst_results: dict, src_results: dict) -> dict:
        ...
        Returns:
            dict: Updated result dict.
        """
        dst_img_vi = dst_results['img_vi']
        dst_img_ir = dst_results['img_ir']
        dst_bboxes = dst_results['gt_bboxes']
        dst_labels = dst_results['gt_bboxes_labels']
        dst_masks = self.get_gt_masks(dst_results)
        dst_ignore_flags = dst_results['gt_ignore_flags']

        src_img_vi = src_results['img_vi']
        src_img_ir = src_results['img_ir']
        src_bboxes = src_results['gt_bboxes']
        src_labels = src_results['gt_bboxes_labels']
        src_masks = src_results['gt_masks']
        src_ignore_flags = src_results['gt_ignore_flags']
        ...
        # Paste source objects to destination image directly
        img_vi = dst_img_vi * (1 - composed_mask[..., np.newaxis]
                         ) + src_img_vi * composed_mask[..., np.newaxis]
        img_ir = dst_img_ir * (1 - composed_mask[..., np.newaxis]
                         ) + src_img_ir * composed_mask[..., np.newaxis]
        bboxes = src_bboxes.cat([updated_dst_bboxes[valid_inds], src_bboxes])
        labels = np.concatenate([dst_labels[valid_inds], src_labels])
        masks = np.concatenate(
            [updated_dst_masks.masks[valid_inds], src_masks.masks])
        ...
        dst_results['img_vi'] = img_vi
        dst_results['img_ir'] = img_ir
        dst_results['gt_bboxes'] = bboxes
        dst_results['gt_bboxes_labels'] = labels
        dst_results['gt_masks'] = BitmapMasks(masks, masks.shape[1],
                                              masks.shape[2])
