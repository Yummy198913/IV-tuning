...
from mmengine.model import BaseDataPreprocessor, ImgDataPreprocessor, ImgDataPreprocessor_vi_ir
...
@MODELS.register_module()
class DetDataPreprocessor_vi_ir(ImgDataPreprocessor_vi_ir):
    """Image pre-processor for detection tasks.
    ...
    def __init__(self,
                 mean_vi: Sequence[Number] = None,
                 mean_ir: Sequence[Number] = None,
                 std_vi: Sequence[Number] = None,
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        """
        batch_pad_shape = self._get_pad_shape(data)
        data = super().forward_vi_ir(data=data, training=training)
        inputs_vi, inputs_ir, data_samples = data['inputs_vi'], data['inputs_ir'], data['data_samples']

        if data_samples is not None:
            # NOTE the batched image size information may be useful, e.g.
            # in DETR, this is needed for the construction of masks, which is
            # then used for the transformer_head.
            batch_input_shape = tuple(inputs_vi[0].size()[-2:])
            for data_sample, pad_shape in zip(data_samples, batch_pad_shape):
                data_sample.set_metainfo({
                    'batch_input_shape': batch_input_shape,
                    'pad_shape': pad_shape
        ...
        if training and self.batch_augments is not None:
            for batch_aug in self.batch_augments:
                inputs_vi, inputs_ir, data_samples = batch_aug(inputs_vi, inputs_ir, data_samples)

        return {'inputs_vi': inputs_vi, 'inputs_ir': inputs_ir, 'data_samples': data_samples}

    def _get_pad_shape(self, data: dict) -> List[tuple]:
        """Get the pad_shape of each image based on data and
        pad_size_divisor."""
        _batch_inputs_vi = data['inputs_vi']
        # Process data with `pseudo_collate`.
        if is_seq_of(_batch_inputs_vi, torch.Tensor):
            batch_pad_shape = []
            for ori_input in _batch_inputs_vi:
                pad_h = int(
                    np.ceil(ori_input.shape[1] /
                            self.pad_size_divisor)) * self.pad_size_divisor
                pad_w = int(
                ...
                batch_pad_shape.append((pad_h, pad_w))
        # Process data with `default_collate`.
        elif isinstance(_batch_inputs_vi, torch.Tensor):
            assert _batch_inputs_vi.dim() == 4, (
                'The input of `ImgDataPreprocessor` should be a NCHW tensor '
                'or a list of tensor, but got a tensor with shape: '
                f'{_batch_inputs_vi.shape}')
            pad_h = int(
                np.ceil(_batch_inputs_vi.shape[2] /
                        self.pad_size_divisor)) * self.pad_size_divisor
            pad_w = int(
                np.ceil(_batch_inputs_vi.shape[3] /
                        self.pad_size_divisor)) * self.pad_size_divisor
            batch_pad_shape = [(pad_h, pad_w)] * _batch_inputs_vi.shape[0]
        else:
            raise TypeError('Output of `cast_data` should be a dict '
                            'or a tuple with inputs and data_samples, but got'
                            f'{type(data)}: {data}')
...
@MODELS.register_module()
class BatchFixedSizePad_vi_ir(nn.Module):
    """Fixed size padding for batch images.
    ...
    def forward(
        ...
        """Pad image, instance masks, segmantic segmentation maps."""
        src_h, src_w = inputs_vi.shape[-2:]
        dst_h, dst_w = self.size

        if src_h >= dst_h and src_w >= dst_w:
            return inputs_vi, inputs_ir, data_samples
