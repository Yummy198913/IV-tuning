...
class HybridAdapter(nn.Module):
    ...
    def forward(self, x, hw_shapes):
        ...
        b, n, c = project1.shape  # 6, 19200, 64
        h, w = hw_shapes  # 120, 160

        project1 = project1.reshape(b, w, h, c).permute(0, 3, 1, 2).contiguous()  # b, c, h, w
        project1 = self.adapter_conv(project1)
        project1 = project1.permute(0, 2, 3, 1).reshape(b, n, c).contiguous()  # b, h, w, c
...
class InternalPromptAdapter(nn.Module):
    ...
    def forward(self, x, hw_shapes):
        ...
        b, n, c = x.shape
        h, w = hw_shapes
        x = x.reshape(b, w, h, c).permute(0, 3, 1, 2).contiguous()  # bchw
        x = self.simam(x)
        x = self.act(x)
        x = x.permute(0, 2, 3, 1).reshape(b, n, c).contiguous()  # bhwc
