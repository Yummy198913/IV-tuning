...
class HybridEmbed(nn.Module):
    ...
    def __init__(self, backbone, img_size=224, feature_size=None, in_chans=3, embed_dim=768):
        ...
        if feature_size is None:
            with torch.no_grad():
                # pFIXME this is hacky, but most reliable way of determining the exact dim of the output feature
...
class ConvMAE(BaseModule):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, img_size=[512, 128, 64], patch_size=[4,2,2], in_chans=3, embed_dim=[256,384,768], depth=[2,2,11],
            ...
            if 'model' in ckpt:
                ...
                for k,v in _state_dict.items():
                    if k in self.state_dict():
                        if _state_dict[k].shape == self.state_dict()[k].shape:
                            keys.append(k)
                new_dict = {k: _state_dict[k] for k in keys}
