...
from zym_tools.wrappers import resize
...
class ViT(BaseModule):
    ...
    def __init__(self,
        ...
        if use_abs_pos:
            ...
            self.num_patches = num_positions
        else:
            self.pos_embed = None
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            ...
            # ckpt = self.resize_abs_pos_embed(ckpt)
            if 'pos_embed' in ckpt.keys():
                if self.pos_embed.shape != ckpt['pos_embed'].shape:
                    logger.info(msg='Resize the pos_embed shape from {} to {}'.
                                format(ckpt['pos_embed'].shape, self.pos_embed.shape))
                    h, w = self.img_size, self.img_size
                    pos_size = int(
                        math.sqrt(ckpt['pos_embed'].shape[1] - 1))
                    ckpt['pos_embed'] = self.resize_pos_embed(ckpt['pos_embed'],
