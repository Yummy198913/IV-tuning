...
data_root = '/raid/zhangyaming2/Datasets/M3FD/'
...
train_pipeline = [
    dict(type='LoadImageFromFile_vi_ir', backend_args=backend_args),
    dict(type='LoadAnnotations', with_bbox=True, with_mask=True),
    dict(type='RandomFlip_vi_ir', prob=0.5),
    dict(
    ...
    dict(type='Pad_vi_ir', size=image_size, pad_val=dict(img=(114, 114, 114))),
    dict(type='PackDetInputs_vi_ir')
]

test_pipeline = [
    dict(type='LoadImageFromFile_vi_ir', backend_args=backend_args),
    dict(type='Resize_vi_ir', scale=image_size, keep_ratio=True),
    dict(type='Pad_vi_ir', size=image_size, pad_val=dict(img=(114, 114, 114))),
    dict(type='LoadAnnotations', with_bbox=True, with_mask=True),
    dict(
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_prefix=dict(img_ir='ir/', img_vi='vi/'),
...
val_evaluator = dict(
    type='CocoMetric',
    ann_file='/raid/zhangyaming2/Datasets/M3FD/annotations_coco/instances_val2017_with_gt_masks.json',
    metric=['bbox', 'segm'],
    format_only=False)
