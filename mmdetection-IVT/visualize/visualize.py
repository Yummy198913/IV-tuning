...
class visualize_mmdetection():
    ...
    def load_data(self):
        ...
        for i in range(1, len(self.dict_list)):
            for value, key in dict(self.dict_list[i]).items():
                ...
                # print(self.dict_list[i])
                if 'loss_rpn_cls' not in self.dict_list[i]:
                    continue
    ...
    def show_chart(self, pathname):
        ...
        # plt.plot(self.acc)
        plt.suptitle((pathname + "\n training result"), fontsize=30)
        plt.savefig(('output/' + pathname + '_result.png'))


if __name__ == '__main__':
    path = '/data/hdd/zhangyaming/Outputs/mmdetectionDM/codetr-swin-l/prompt/20240802_141335/vis_data/20240802_141335.json'
    # x = visualize_mmdetection(sys.argv[1])
    x = visualize_mmdetection(path)
    file_name = os.path.basename(path)
    file_name = file_name.split('_')[0] + '_' + file_name.split('_')[1]
    x.load_data()
    x.show_chart(file_name)