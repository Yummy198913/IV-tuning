...
class LoadImageFromFile_vi_ir(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        filename_vi = results['img_path_vi']
        filename_ir = results['img_path_ir']
        try:
            if self.file_client_args is not None:
                file_client_vi = fileio.FileClient.infer_client(
                    self.file_client_args, filename_vi)
                file_client_ir = fileio.FileClient.infer_client(
                    self.file_client_args, filename_ir)
                img_bytes_vi = file_client_vi.get(filename_vi)
                img_bytes_ir = file_client_ir.get(filename_ir)
            else:
                img_bytes_vi = fileio.get(
                    filename_vi, backend_args=self.backend_args)
                img_bytes_ir = fileio.get(
                    filename_ir, backend_args=self.backend_args)
            img_vi = mmcv.imfrombytes(
                img_bytes_vi, flag=self.color_type, backend=self.imdecode_backend)
        ...
        assert img_vi is not None, f'failed to load image: {filename_vi}'
        if self.to_float32:
            img_vi = img_vi.astype(np.float32)
        ...
        results['img_shape'] = img_vi.shape[:2]
        results['ori_shape'] = img_vi.shape[:2]
        return results
