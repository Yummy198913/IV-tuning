...
from zym_tools.IV_tuning import *
...
class SwinBlock(BaseModule):
    ...
    def __init__(self,
        ...
        self.hmps_prompt = HMPS(embed_dims, 64)  # fixme

    def forward(self, x, x1, hw_shape):

        def _inner_forward(x, x1):
            H, W = hw_shape
            ...
            adapt_x = self.hmps_prompt(x, x1, (H, W))
            x = x + adapt_x
        ...
        if self.with_cp and x.requires_grad:
            x = cp.checkpoint(_inner_forward, x, x1)
        else:
            x = _inner_forward(x, x1)
...
class SwinTransformer_prompt_simple(BaseModule):
    """ Swin Transformer
    ...
    def __init__(self,
        ...
        super(SwinTransformer_prompt_simple, self).__init__(init_cfg=init_cfg)
        ...
        self.patch_embed_prompt = PatchEmbed(
            ...
            init_cfg=None)
        '''prompt parameters'''
        self.hmp_prompt = HMP(inplanes=embed_dims, hide_channel=8)
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformer_prompt_simple, self).train(mode)
        self._freeze_stages()
    ...
    def forward(self, x, z):
        ...
        z, hw_shape_z = self.patch_embed_prompt(z)

        '''prompt at input'''
        x_feat = self.hmp_prompt(x, z, hw_shape_z)  # 1, 1024, 32, 32
        x_prompted = feature2token(x_feat)  # 1, 1024, 1024
        x = x + x_prompted
        ...
        for i, stage in enumerate(self.stages):
            x, hw_shape, out, out_hw_shape = stage(x, x_prompted, hw_shape)
            x_prompted = x
