...
from .beit_util import BEiT, BEiTAttention, BEiTTransformerEncoderLayer
...
class MAE(BEiT):
    ...
    def init_weights(self):
        ...
        # self.init_cfg = {'checkpoint': '/data2/chenhao/zhangyaming/Projects/mmsegmentationDM/pretrain/mae_pretrain_vit_large_mmseg.pth',
        #                  'type': 'Pretrained'}
        ...
        path = '/data2/chenhao/zhangyaming/Projects/mmsegmentationDM/pretrain/mae_pretrain_vit_large_mmseg.pth'
        checkpoint = _load_checkpoint(
            path, logger=None, map_location='cpu')
        state_dict = self.resize_rel_pos_embed(checkpoint)
        state_dict = self.resize_abs_pos_embed(state_dict)
        msg = self.load_state_dict(state_dict, False)
        print(msg)
    ...
    def forward(self, img_vi, img_ir):
        ...
        x, hw_shape_x = self.patch_embed(img_vi)
        # z, hw_shape_z = self.patch_embed_prompt(img_ir)

        # stole cls_tokens impl from Phil Wang, thanks
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        x = x + self.pos_embed  # X:1, 1025, 768; POS: 1, 1601, 768

        outs = []
        for i, layer in enumerate(self.layers):
            x = layer(x)  # b, 1025, 1024
            ...
            if i in self.out_indices:
                out = x[:, 1:]
                B, _, C = out.shape
                out = out.reshape(B, hw_shape_x[0], hw_shape_x[1],
                                  C).permute(0, 3, 1, 2).contiguous()
                outs.append(out)
        outs[0] = F.interpolate(
            outs[0], scale_factor=4, mode="bilinear", align_corners=False
        )
        outs[1] = F.interpolate(
            outs[1], scale_factor=2, mode="bilinear", align_corners=False
        )
        outs[3] = F.interpolate(
            outs[3], scale_factor=0.5, mode="bilinear", align_corners=False
        )
