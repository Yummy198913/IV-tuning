...
class VisionTransformer(BaseModule):
    ...
    def init_weights(self):
        ...
        txt_path = ('/public/data3/zhangyaming/Projects/mmsegmentation/zym_log/'
                    '{}/param.txt').format(os.path.basename((__file__).split('.')[0]))
        if 'all' in self.frozen_exclude:
            ...
            return
        if 'freeze' in self.frozen_exclude:
    ...
    def forward(self, img_vi, img_ir):
        ...
        B = img_vi.shape[0]
        # hw_shape: (32, 32)
        x, hw_shape = self.patch_embed(img_vi)  # 1, 1024, 1024
        z, hw_shape = self.patch_embed_prompt(img_ir)  # 1, 1024, 1024
        x = x + z  # simple feature fusion

        # stole cls_tokens impl from Phil Wang, thanks
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        x = self._pos_embeding(x, hw_shape, self.pos_embed)  # 1, 1025, 1024

        if not self.with_cls_token:  # self.with_cls_token:True
            # Remove class token for transformer encoder input
            x = x[:, 1:]  # 1, 1024, 1024

        if self.pre_norm:
            x = self.pre_ln(x)  # 1, 1025, 1024

        outs = []
        if self.out_origin:  # False
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x)  # 1, 1025, 1024
            if i == len(self.layers) - 1:
                if self.final_norm:  # False
                    x = self.norm1(x)
            if i in self.out_indices:  # out_dices[9, 14, 19, 23]
                if self.with_cls_token:  # True
                    # Remove class token and reshape token for decoder head
                    out = x[:, 1:]  # 1, 1024, 1024
                else:
                    out = x
                ...
                out = out.reshape(B, hw_shape[0], hw_shape[1],
                                  C).permute(0, 3, 1, 2).contiguous()
                if self.output_cls_token:  # False
                    out = [out, x[:, 0]]
                outs.append(out)
