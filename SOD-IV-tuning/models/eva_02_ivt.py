...
class Block(nn.Module):
    def __init__(
        ...
        self.hmps_ivt = HMPS(dim, 64)

    def forward(self, x, x1, rel_pos_bias=None):
        L = x.shape[1]
        H = W = int((L - 1) ** 0.5)
        ...
        if self.gamma_1 is None:
            ...
            # hybrid adapter with internal prompt: 1
            cls_token = x[:, :1, :]
            x_wo_cls = x[:, 1:, :]
            adapt_x = self.hmps_ivt(x_wo_cls, x1, (H, W))
            x_wo_cls = x_wo_cls + adapt_x
            x = torch.cat([cls_token, x_wo_cls], dim=1)

            x = x + self.drop_path(self.mlp(self.norm2(x)))

            # hybrid adapter with internal prompt: 2
            cls_token = x[:, :1, :]
            x_wo_cls = x[:, 1:, :]
            adapt_x = self.hmps_ivt(x_wo_cls, x1, (H, W))
            x_wo_cls = x_wo_cls + adapt_x
...
class EVA2_ivt_simple(nn.Module):
    ...
    def __init__(
        ...
        if hybrid_backbone is not None:
            ...
            self.patch_embed_ivt = PatchEmbed(
                img_size=img_size,
                patch_size=patch_size,
                in_chans=in_chans,
                embed_dim=embed_dim,
        ...
        '''EMPG'''
        self.hmp_ivt = HMP(inplanes=embed_dim, hide_channel=8)
    ...
    def forward_features(self, x, z):
        ...
        z, (Hp, Wp) = self.patch_embed_ivt(z)
        batch_size, seq_len, _ = x.size()

        '''EMPG prompt'''
        x_feat = self.hmp_ivt(x, z, (Hp, Wp))  # 1, 1024, 32, 32
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_checkpoint:
                ...
                x = blk(x, x_prompted, rel_pos_bias)
                x_prompted = x.split((1, x.shape[1] - 1), dim=1)[1]
