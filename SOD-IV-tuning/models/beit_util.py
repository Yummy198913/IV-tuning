...
from .vit_mona8 import TransformerEncoderLayer as VisionTransformerEncoderLayer, Mona, ExtraAdapter
...
class BEiTTransformerEncoderLayer(VisionTransformerEncoderLayer):
    ...
    def __init__(self,
        ...
        self.gamma_2 = nn.Parameter(
            init_values * torch.ones(embed_dims), requires_grad=True)
        self.HA1_ivt = Mona(embed_dims, 64)
        self.HA2_ivt = Mona(embed_dims, 64)
        self.IPA1_ivt = ExtraAdapter(embed_dims, 8)
        self.IPA2_ivt = ExtraAdapter(embed_dims, 8)
    ...
    def forward(self, x, x1):
        ...
        cls_token = x[:, :1, :]
        x_wo_cls = x[:, 1:, :]
        x = self.HA1_ivt(x_wo_cls, (H, W))
        x = x + self.IPA1_ivt(x1, (H, W))
        ...
        cls_token = x[:, :1, :]
        x_wo_cls = x[:, 1:, :]
        x = self.HA2_ivt(x_wo_cls, (H, W))
        x = x + self.IPA2_ivt(x1, (H, W))
        x = torch.cat([cls_token, x], dim=1)
        return x
...
class BEiT(BaseModule):
    ...
    def forward(self, img_vi, img_ir):
        ...
        z, hw_shape_az = self.patch_embed_inf(img_ir)
